    ...
    let notificationsMarkup =
      notifications && notifications.length > 0 ? (
        notifications.map((not) => {
          const verb  = not.type === 'like' ? 'Liked your post' : not.type === 'follow' ? 'Started following you' : 'Commented on your post';
          const title = `${not.sender.name} @${not.sender.username}`;
          if(not.type==='comment')
            var time = `(${dayjs(not.createdAt).fromNow()})`
          else
            var time = ` ${verb} (${dayjs(not.createdAt).fromNow()})`;
          ...
          return (
              ...
              <ListItem>
                <ListItemAvatar>
                  <Avatar src={not.sender.avatar}  className={classes.bigAvatar} />
                </ListItemAvatar>
                <ListItemText className={classes.text} primary={title} secondary={
                  <React.Fragment>
                  <Typography
                    component="span"
                    variant="body2"
                ...
                <ListItemSecondaryAction>
                  <IconButton edge="end" aria-label="delete">
                    {not.type != 'follow' && not.image ?<Avatar variant="square" src={not.image}  className={classes.smallAvatar} />:''}
                  </IconButton>
                </ListItemSecondaryAction>
              </ListItem>
